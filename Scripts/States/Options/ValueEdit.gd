...
func change_value():
	...
	if (valueEditing is bool):
		if (Input.is_action_just_pressed("right") || Input.is_action_just_pressed("left")):
			...
			optionsMenu.get_node("Sounds/ScrollStream").play()
...
func set_option():
	optionsMenu.get_node("Sounds/ConfirmStream").play()
...
func edit_value(_name):
	optionsMenu.get_node("Sounds/PopUpStream").play()
	visible = true
